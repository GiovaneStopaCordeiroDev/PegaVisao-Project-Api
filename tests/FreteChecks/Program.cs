...
Rejeita(() => FreteRegras.PrepararItem(new() { Estoque = 10, Produto = new() { Nome = "Sem medidas", Preco = 30 } }, 1), 422, "Produto sem medidas");
var produto = new Produto { Nome = "Teste", Preco = 30, PesoKg = .5m, AlturaCm = 5, LarguraCm = 20, ComprimentoCm = 30 };
var item = FreteRegras.PrepararItem(new() { Id = 1, Estoque = 10, Produto = produto }, 2);
...
var semEntrega = FreteRegras.PrepararItem(new() { Id = 2, Estoque = 10, Produto = new() { Nome = "Teste", Preco = 1m } }, 1, exigirMedidas: false);
Check(semEntrega.Preco == 1m && semEntrega.PesoKg == 0m, "Modo sem entrega preserva preco sem exigir medidas");
Rejeita(() => FreteRegras.PrepararItem(new() { Estoque = 10, Produto = new() { Preco = 0m } }, 1, exigirMedidas: false), 422, "Modo sem entrega rejeita preco invalido");
var semFreteCotacao = new CotacaoFrete { UsuarioId = 1, CepDestino = "01001000", CarrinhoHash = hash, ConexaoId = "sem-frete-teste", ExpiraEm = agora.AddMinutes(15) };
Rejeita(() => FreteRegras.ValidarCotacao(semFreteCotacao, 1, "01001000", hash, "sandbox:1", agora), 409, "Reativacao do frete invalida cotacao sem entrega");
